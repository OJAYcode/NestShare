...
namespace Thrift.Controllers
{
    ...
    public class LoanController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateLoanViewModel model)
        {
            ...
            var mongoContext = HttpContext.RequestServices.GetService(typeof(MongoDbContext)) as MongoDbContext;
            if (mongoContext != null && !mongoContext.IsConnected)
            {
                ModelState.AddModelError("", "Cannot save loan: Database is not connected. Please check MongoDB connection.");
                return View(model);
            }
            ...
            loan.UpdatedAt = DateTime.UtcNow;            try
            {
            ...
            catch (Exception ex)
            {
                ...
                ModelState.AddModelError("", $"Error saving loan: {ex.Message}. Please check database connection.");
                return View(model);
            }
        }
